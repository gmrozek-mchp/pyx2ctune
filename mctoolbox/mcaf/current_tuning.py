...
class CurrentTuning(_interfaces.LoopTuner):
    ...
    fixed-point counts via the pymcaf Motor interface.
    """
    ...
    def amps_to_counts(self, amps: float) -> int:
        ...
        return self._session.conn.engineering_to_q15(amps, _PARAM_FULLSCALE_CURRENT)

    def counts_to_amps(self, counts: int) -> float:
        """Convert Q15 counts to a current amplitude in Amps."""
        return self._session.conn.q15_to_engineering(counts, _PARAM_FULLSCALE_CURRENT)
    ...
    def setup_step_test(
        ...
        motor = conn.motor
        sqwave = conn.test_harness.sqwave

        motor.idq_cmd_raw = DQPair(d=0.0, q=0.0)
